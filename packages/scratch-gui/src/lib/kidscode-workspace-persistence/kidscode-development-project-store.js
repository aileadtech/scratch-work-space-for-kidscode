 ...
 * Workspace that touches IndexedDB — the persistence adapter and the Workspace UI never do.
 *
 * Records are keyed by project_ref and hold only the binary project and its version metadata.
 * Launch tokens, workspace access tokens, and student authentication are never written here.
 */
...
/**
 * Create a project store backed by the browser's IndexedDB. Each record is
 * {projectRef, versionRef, versionNumber, savedAt, sb3}.
...
const createIndexedDbProjectStore = () => {
    ...
    return {
        ...
        putProject: record => withStore('readwrite', store => store.put(record)).then(() => {})
    };
};
